...
Changes to the paper are highlighted in red (is it red or blue? DC)
...
**Response**: We agree that the two arguments R and half range can be confusing, but need to keep them for consistency with other display functions in the tourr package. While R is the appropriate argument to use with the sage display function (setting the range for the re-scaling of the radius), the half range argument is part of the tour display function and used to set the scale on the plotting canvas. Indeed it is a function of the data, but in general tourr display functions allow resetting this argument to allow the user to zoom and pan. We have rephrased the paragraph to make this clearer. 

The scaling factor of 0.9 is an internal re-scaling before drawing the points on the canvas and is not important for understanding the method. To avoid confusion we have removed the corresponding statement.
...
XXX should we include the mathematical proof somewhere? Don't really want to add an appendix for this, but not really something to include in the main text either... DC says not necessary to include
...
**Response**: We agree that there is an overall effect of reducing density, or we can think of it as adjusting the resolution according to the position in the plane. Note however, that the effect is the same within and between clusters. Thus, if points within a cluster remain grouped while separation between clusters becomes more apparent, this can be interpreted as a true finding. Conversely, in the case of a visual artifact we expect a general reduction of density without separating the clusters. We have added this argument to the discussion section.
...
**Response**: XXX I guess we can remove this? I think it could be a footnote instead. DC says, I'm reluctant to remove it because it is a nod to the large body of work on sparsity, and helps put our work in context. Maybe we can start the sentence with "Although it is irrelevant for the topic of this paper, it is interesting to note that Donoho ..."
...
Maybe this ref would be ok? there is no full reference according to Donoho's homepage, but the pdf of the paper is available in multiple places.
...
@misc{Donoho00high-dimensionaldata,
    ...
    note = {Unpublished paper presented at AMS Conference on Math Challenges of the 21st Century},
    year = {2000},
...
**Response**: It would be section 5.5 of Chang. It's good to keep this, but should be made clear that this has a good and accessible discussion of what is overplotting and how to alleviate it. 

This is an original reference to overplotting, and fixing it with density displays. 

Carr, D., Littlefield, R., Nicholson, W., & Littlefield, J. (1987). Scatterplot Matrix Techniques for Large N. Journal of the American Statistical Association, 82(398), 424-436. doi:10.2307/2289444

We could add citations to be polite:
...
BUT, I see more infovis references preceding this one. Let's add Carr reference, and make it clear the Chang is an accessible discussion with code. 
...
XXX say more here? I wouldn't mind removing either, but remember that it did lead to confusion before (when not explaining exactly what was meant here...) DC says, yes, leave it and your response is good.
...
**Response**: XXX this is problem in the bib entries, need to fix by hand. Do we want to go with full journal names everywhere? DC says we should probably spell them all out in the .bib file. Later the copy editor might make changes.
...
XXX what more should we say here? DC: The important part of the PCA transformation is to remove the correlation between variables while keeping other structures intact. The individual scale on variable axes is irrelevant. Once a number of PCs is chosen the decision has been made that dimensions dropped are considered to have insufficient variation. The relative importance of each PC kept for analysis is ignored because it is irrelevant. 
...
**Response**: DC: We really appreciate the acronym provided by the reviewer! I like it just as is! And would make a note of this acronym in the Acknowledgements section.
...
**Response**: DC: We appreciate this point of view, and do also think this is interesting but don't know what to do with it for this paper. We think it is best left for us all to ponder on for now.
...
XXX add to discussion DC: agree
