...
\textcolor{blue}{(Note that, sampling from a uniform in a sphere is used for this illustration for several reasons. Any linear projection is equivalent because of the rotation invariance of the distribution. The uniform sampling places focus on the space, points are spread evenly across the volume, and not a data distribution. Also, $p=10$ would already be considered high-dimensional, as seen by the piling observed in the right-most plot.)}
