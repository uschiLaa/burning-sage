...
While the still views look very similar, comparing the tour animations (available in the supplementary material) reveals some differences between the display with trimming or tuning. When trimming by setting $R=1$ the focus is clearly on the center of the distribution, and most points get pushed out towards the circle of this maximum radius. On the other hand, tuning the display by setting $\gamma=20$ preserves the elliptical shape of the distribution, making it is easier to see correlation patterns.
...
by a simple function (Eq. \ref{eq:resc}).
