...
In this work we address the piling in low-dimensional linear projections, which is in particular a barrier for viewing high-dimensional distributions with tour methods [@As85,@BCAH05]. Tours show interpolated sequences of low-dimensional projections of the data. When exploring a dataset with a tour we look at the distribution from different sides and want to discover features that may be visible from a specific viewing angle. However, these features may easily be hidden if most of the observations pile up near the center, while a large fraction of the plotting canvas will only show sparse outlying points.

Here we suggest to redistribute projected data points such that equal volume in the original high-dimensional space will approximately be mapped onto equal volume in the low-dimensional projection, focussing on equal area in a 2D projection plane. This redistribution is achieved by assuming spherical distributions, and deriving a non-linear transformation of the projected radius. This essentially results in zooming in near the center of the distribution, where the transformation is approximately linear, while points that are further out in the projection get pushed towards the maximum radius.
...
XXX show density contours, similar to Fig. 3
