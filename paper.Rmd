...
@Donoho00 considers the curse of dimensionality as a blessing, because the sparsity can be leveraged for computational efficiency. This is used in regularization methods, like lasso, to penalize model complexity. The penalty term results in shrinking (some of) the parameter estimates towards zero.

Paradoxically, the curse of dimensionality inverts for dimension reduction, resulting in an excessive amount of observations near the center of the distribution. This affects visualizations made on low-dimensional projections, like the tour [@As85,@BCAH05]. The effect is described by @diaconis1984, that most low-dimensional linear projections will show approximately Gaussian distributions of the data, with observations concentrating in the center. This motivated the development of indexes for projection pursuit searching for departure from normality. It is also related to what is called "data piling" in high-dimension low-sample size data [@10.2307/27639976,@10.1093/biomet/asp084]: all observations are in a single point in certain low-dimensional projections.  These issues also persist with non-linear dimension reduction techniques, and referred to as the "crowding problem", which methods like t-Distributed Stochastic Neighbor Embedding (t-SNE) [@tsne] aim to alleviate.

Figure \ref{fig:density} illustrates the crowding problem. Two-dimensional linear projections of points sampled uniformly within $p$-dimensional hyperspheres in $p=3, 10, 100$ are displayed as hexbin plots. Color indicates log count of the bin, with yellow mapping the highest counts. As $p$ increases the density concentrates in the center of the projection. 
...
In this work we address data crowding in low-dimensional linear projections by providing a reverse transformation for tour methods. Tours show interpolated sequences of low-dimensional projections of the data. When exploring a dataset with a tour we can discover features that are only visible from specific viewing angles. However, these features could be hidden in the observation pile, and even worse leaving a large fraction of the available plotting canvas sparsely populated. To reverse the effect, we introduce a radial transformation that magnifies the center of the distribution. This is called a burning sage tour, to reflect that the curse of dimensionality is being removed.

The paper is structured as follows. The radial transformation and its implementation is described in Section \ref{sec:method}. Section \ref{sec:application} illustrates the use of the sage tour with examples in clustering, supervised classification and a classic needle-in-the-haystack problem.
...
detect cell types and find expression of genes that define those cell types, and the relative orientation of the cell types  to each other (trajectory analysis) [@Amezquita2020-at]. Generally, for cluster verification, analysts use embedding methods
...
(described by @Amezquita2020-at): we performed quality control using the `scater` package [@McCarthy2017]
and we log-transformed and normalised the expression 
values and selected highly variable genes (HVGs) using `scran` 
[@Lun2016]. The top ten percent of most HVGs were used as features to subset
the normalised expression matrix and compute principal components.
...
Figure \ref{fig:mouse-cluster} shows selected frames from the animations available in the supplementary material. The still views serve to illustrate the main points, but we encourage the reader to look at the tour animations to fully appreciate the advantage of the sage display.
...
```{r sketches, fig.cap = "Selected frames  of the tour run on the sketches data using the default tour display (top), and using the default sage display with $\\gamma=2$ (bottom). Three types of sketches are indicated by color: banana (green), cactus (orange) and crab (purple). Overplotting of points is a problem for the grand tour display, while the sage display reveals low density near the center.", out.width="0.9\\textwidth", fig.align="center"}
...
Following the processing described there, we tour the first 6 principal components, rescaled to have zero mean and unit variance. In Figure \ref{fig:pdfsense} we see that the sage display with $R = 10$ (Figure \ref{fig:pdfsense}, bottom), maintains the overall shape of the data seen using the default tour display (Figure \ref{fig:pdfsense}, top). The different physical process, shown in different colors, are indeed orthogonal in the parameter space, as can be seen most clearely by looking at the animations  available in the supplementary material.
...
Neither the standard scatter display nor the sage display with default settings can reveal the structure, see still view from the sage display with default settings in Figure \ref{fig:pollen} (left). This view uses the maximum radius in the scaled data, thus $R=6.6$, and no tuning of $p_{\mathrm{eff}}$, thus $\gamma=1$. We can use either $\gamma$, $R$ or a combination of the two to zoom in further near the center, for example we use trimming $(R=1, \gamma=1)$ or tuning $(R=6.6,\gamma=20)$ in the middle and right view shown in Figure \ref{fig:pollen}. There is approximate equivalence between the results obtained using either tuning or trimming, and both views clearly reveal the word "EUREKA" hidden in the distribution.
...
While the still views look very similar, comparing the tour animations (available in the supplementary material) revelas some differences between the display with trimming or tuning. When trimming by setting $R=1$ the focus is clearely on the center of the distribution, and most points get pushed out towards the circle of this maximum radius. On the other hand, tuning the display by setting $\gamma=20$ preserves the elliptical shape of the distribution, making it is easier to see correlation patterns.

# Discussion {#sec:concl}

In this paper we have introduced the sage display, a visualisation technique for reversing
the curse of dimensionality in low dimensional projections of high dimensional data, and have shown its utility in applications in the natural sciences and machine learning. 
The new method combines animated linear dimension reduction obtained with a grand tour and a non-linear transformation of the projected data points to improve intelligibility of each view.
Our case studies show how it reduces the effects of data piling and aids different tasks in exploratory high dimensional data analysis.

The approach  provides interpretable visualisation that captures high dimensional information and preserves global structure, and it is complementary to non-linear dimension reduction techniques. For example, when visualising clusters, the sage display enables an assessment of cluster sizes, and accurately captures relative position and orientation. However, the sage display is currently 
naive to information in high dimensional space that is not 
captured by linear projections. Like t-SNE, it would be interesting to explore
transformations of projections where points in the high dimensional space are 
locally dense instead of expanding from the center of the distribution.

XXX option to reference upcoming work in liminal paper when mentioning complementarity

While the recently introduced slice tour [@sliceTour] is essentially addressing the same issue as the sage display (i.e. overplotting of points from the orthogonal space), it follows a different philosophy: we only select a small portion of the volume for plotting, based on the orthogonal distance of points from the projection plane. This approach is preferred for large sample sizes, or for concave structure, while the sage display works better in the case of limited sample size.

The sage display is easily incorporated into exploratory high dimensional data analysis, and applications shown in Section \ref{sec:application} provide examples of the following tasks: 

- classification tasks: the sage display decreases the number of overlapping points between classes and provides better visual separation between classes compared to the regular tour, as shown in the sketches example (Section \ref{sec:appl2})
- clustering tasks: the sage display uncovered clusters that were originally obscured by data piling, while still giving the viewer an accurate assessment of the size of a cluster, and their relative orientation, as shown in the single cell RNA-seq example (Section \ref{sec:appl1})
- shape analysis tasks: the sage display helps us understand structures across multiple dimensions, as shown in the pdfsense exapmple (Section \ref{sec:appl3}). By zooming in on the  points in the blue cluster we saw how they were present in all directions of the parameter space, which was difficult to see with the regular tour.
- search tasks: the sage display allows to find hidden signal that is concealed by the density of points around the center of the projection, as shown in the pollen example (Section \ref{sec:appl4})

The applications also show that tuning is often required,
especially if the projection is dense in the center of the 
distribution. Expanding around the center aggressively can introduce distortions
in the display, a well known problem from non-linear dimension reduction techniques like t-SNE.
However, unlike those methods, the parameters in the sage display are 
interpretable; we know the effect of altering $\gamma$ and $R$ on the resulting 
visualisation. Recall that the radial transformation is approximately linear near
the center, thus preserving structures such as those seen in the pdfsense and
pollen applications. On the other hand, the tuning example showed how points away
from the projected center get moved to the edge of the hypersphere as $\gamma$
is increased or $R$ is decreased. 

Given
that the display is quick to compute, being able to modify these parameters in 
real time with an interactive interface would be beneficial.
...
The source material for this paper is available at \url{https://github.com/uschiLaa/burning-sage} XXX rename repo!!!!. The animated gifs for all applications are also included in html files in the supplementary material.
