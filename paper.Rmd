...
Using the default tour display (figure \ref{fig:mouse-cluster}, top), the three clusters (dark green, blue, and yellow) are obscured by points in other clusters as we move through the frames of the animation. The points in dark green cluster are overlapping those found in the yellow and blue clusterss; and it is difficult to see if there is any separation between the blue and yellow clusters.

```{r mouse-cluster, fig.cap = "Selected frames from using a tour of the mouse data with the default tour display (top), and the sage display with $\gamma=3$ (bottom)", fig.width="0.9\\textwidth"}
library(cowplot)
dir <- "pngs/"
frames <- c("001", "016", "038", "074")
draw_panel <- function(png) ggdraw() + 
  draw_image(png) + 
  theme(plot.margin = unit(c(0, 0, 0, 0), "cm"))

mex_grand <- paste0(dir, "mouse_grand_2c-", frames, ".png")
mex_grand_panel <- plot_grid(plotlist = lapply(mex_grand, draw_panel), nrow = 1)
mex_sage <- paste0(dir, "mouse_sage_2c_gam3-", frames, ".png")

mex_sage_panel <- plot_grid(plotlist = lapply(mex_sage, draw_panel), nrow = 1)

plot_grid(mex_grand_panel, mex_sage_panel, nrow = 2)
```
...
Figure \ref{fig:sketches} shows the grand tour on the PCs, where green points correspond to the banana class, orange points represent the cactcus class and purple points are the crab class. In both displays points belonging to the cactus class are concentrated in the center, however on the default display (figure \ref{fig:sketches}, top) there is overplotting: points from other classes overlap those in the cactus class. The sage display (figure \ref{fig:sketches}, right) helps reduce overplotting, it is easier to see that the centers of class are separated and that there is substructure in the banana class, which further collapses into two subgroups.


```{r sketches, fig.cap = "Selected frames  of the tour run on the sketches data using the default tour display (top), and using the default sage display (bottom).", fig.width="0.9\\textwidth"}
frames <- c("001", "036", "077", "092")

sketches_grand <- paste0(dir, "sketches_grand-", frames, ".png")
sketches_grand_panel <- plot_grid(plotlist = lapply(sketches_grand, draw_panel), nrow = 1)
sketches_sage <- paste0(dir, "sketches_sage-", frames, ".png")

sketches_sage_panel <- plot_grid(plotlist = lapply(sketches_sage, draw_panel), nrow = 1)

plot_grid(sketches_grand_panel, sketches_sage_panel, nrow = 2)
```
