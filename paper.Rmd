...
The transformation is defined for projected data points in the plane, $x = (x_1, x_2)$, and we work with polar coordinates in the plane, $(r_x, \theta_x)$. The angular component $\theta_x$ is uniform for this distribution, by the rotation invariance of the sphere, and thus does not need to be transformed. The radial component $r_x$ is transformed in two steps. We first transform $r_x$ via the CDF, i.e. we compute $v_{2} (r_x, p, R)$, which is uniformly distributed in radius when the observations $x$ are indeed uniformly distributed in the $p$ dimensional hypersphere of radius $R$. We then transform $v_{2} (r_x, p, R)$ using the inverse of $v_{2} (r_x, 2, R)$, to go from a uniform distribution in radius to a uniform distribution across the area of the disk (notice that we have fixed $p=2$ here). This inverese is defined via
\begin{equation}
...
The radial transformation to go from a uniform distribution in a $p$ dimensional hypersphere to a uniform distribution in a $2$ dimensional disk is therefore given by 
\begin{equation}
...
```{r radii, fig.cap="Relation between $r_x$ and $r'_x$ for different values of $p$. The scaling is approximately linear near the center, but leads to distortion at large radii when $p$ is large.", fig.height=3, fig.width=6, out.width="60%", fig.align = "center"}
