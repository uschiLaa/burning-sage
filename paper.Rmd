...
abstract: |
  In high-dimensional data analysis the curse of dimensionality observes that points tend to be far away from the center of the distribution and on the edge of high-dimensional space. However, if the data is projected onto a lower dimensional subspace the inverse happens; projected points tend to clump at the center obscuring any resulting visualisations. We propose to reverse the curse using radial transformations of the projections obtained using the grand tour, called the burning sage algorithm. We apply our algorithm in several case studies that show how our sage visualisations enhance exploratory clustering and classification workflows.
keywords: |
...
- use four panel set up for grand tour and sage
- render pngs in addition to the gifs
...
on a weighted subsample of cells based on their cluster membership - 4,590 cells. When running the sage display, we set $\gamma = 3$ as the effective dimensionality of the data. 


Using the default tour display, the highlighted cluster in dark green is obscured by points in nearby clusters as shown in figure \ref{fig:mouse} (left). Using the sage display, expands the center of projection, so the dark green cluster becomes more visible; the outlier cell that is far away from the rest of the clustered points is more apparent.

- better idea of size of cluster cause not bunching 
- rescale = FALSE, use principal components
- try multiple overlapping cluster example


\begin{figure}
\centering
\includegraphics[width=0.45\textwidth]{png/mouse_grand-38.png}
\includegraphics[width=0.45\textwidth]{png/mouse_sage_gam03-38.png}
\caption{Selected views of the same frame using a tour of the mouse data in the default tour display (left), and using the sage display setting $\gamma=3$ (right).}
\label{fig:mouse}
\end{figure}
...
We can use the new display to look at different distributions of images from Google QuickDraw collection [@quickdraw-paper]. These are $28\times28=784$ pixel grey scale data that are available publicly. In this example, we sample 1000 images from three types of sketches (banana, cactus, crab) and observations  and see if we can separate the classes in the high-dimensional parameter space. 
...
Figure \ref{fig:sketches} shows the grand tour on the PCs, where green points correspond to the banana class, orange points represent the cactcus class and purple points are the crab class. In both displays points belonging to the cactus class are concentrated in the center, however on the default display (figure \ref{fig:sketches}, left) there is overplotting: points from other classes overlap those in the cactus class. The sage display (figure \ref{fig:sketches}, right) helps reduce overplotting, it is easier to see that the centers of class are separated and that there is substructure in the banana class, which further collapses into two subgroups.

\begin{figure}
\centering
\includegraphics[width=0.45\textwidth]{png/sketches_grand-01.png}
\includegraphics[width=0.45\textwidth]{png/sketches_sage-36.png}
\caption{Selected views of the tour run on the sketches data using the default tour display (left), and using the default sage display (right).}
\label{fig:sketches}
\end{figure}

## Comparing physics experiments: PDFSense

Data were obtained from CT14HERA2 parton distribution function fits as used in [reference]. There are 28 directions in the parameter
space of parton distribution function fit, each point in the variables labelled X1-X56 indicate moving 1 standard devation from the 'best'
(maximum likelihood estimate) fit of the function in either positive or negative direction. Each observation has all predictions of the corresponding measurement from an experiment. Each point
is colored according to different underlying processes in the experiment. The goal of the analysis is to identify the trajectory of the experiments along the parameter space corresponding to a given process.

Following the processing described in [@ ursula's paper ], we tour the first 6 principal components, rescaled to have zero mean and unit variance. In figure \ref{fig:pdfsense} we see that using the sage display with $R = 10$ (figure \ref{fig:pdfsense}, right), expands the center of the distribution compared to the regular tour display  (figure \ref{fig:pdfsense}, left), while maintaining the global structure of the data (different physical process are orthogonal in the paramater space). Here using the sage display allows us to see more detail at the center, and identifies parameters that are at intersection of all three physical processes.  

\begin{figure}
\centering
\includegraphics[width=0.45\textwidth]{png/pdfsense_grand-01.png}
\includegraphics[width=0.45\textwidth]{png/pdfsense_sage_R10-51.png}
\caption{Selected views of the tour run on the pdfsense data using the default tour display (left), and using the sage display with $R=10$ (right).}
\label{fig:pdfsense}
\end{figure}
...
non-linear dimension reduction (NLDR) techniques like t-SNE, the sage display still gives the viewer an accurate assessment of the size of a cluster, as we saw in our single cell example. Our parameter tuning example highlights that the sage display is linear up to a given effective dimension. As a result, the sage dispaly provides interpretable visualisations while preserving global structure in the high dimensional space, as highlighted in the particle physics example.

For the sage display tuning is often required to avoid bunching points in the visualisation, especially if the projection is dense in the center of the distribution. However, given that the display is quick to compute, it is easy to try out different parameter combinations. Expanding around the center too aggresively can also introduce distortions in the display, which is also a problem for NLDR methods.

Currently, the sage display is naive information in high dimensional space that is not captured by linear projections. Further work is required to investigate other transformations of the projections that can capture non-linear structure or that are locally linear, like the transformations performed by t-SNE. 

* short summary of what you've done; introduced the sage display to 'reverse' the curse and applied it to several datasets in natural sciences and machine learning...

* pros of the proposed technique: alleviates overplotting issues,  better seperation of clusters and size of clusters; linear up to a point, point to pollen example; very quick to compute and still interpretable; point to examples here
How this would be applied in practice: clustering, classification?

* limitations of the proposed technique: finding appropriate parameters for data; acknowledge bunching but we are not able to capture what is happening - ignoring information in the high-d if you didn't rely on linear projections; possible introduces distortions point back that is also a problem for NLDR techniques but we see where it is happening 

* further work or investigation: 
What other transformations on the projected data make sense?

Relationship with slice tour?

Relationship with nonlinear transformations such as tSNE?
