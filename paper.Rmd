...
Following the processing described there, we tour the first 6 principal components, rescaled to have zero mean and unit variance. In figure \ref{fig:pdfsense} we see that using the sage display with $R = 10$ (figure \ref{fig:pdfsense}, right), expands the center of the distribution compared to the regular tour display  (figure \ref{fig:pdfsense}, left), while maintaining the global structure of the data (different physical process are orthogonal in the paramater space). Here using the sage display allows us to see more detail at the center, and identifies parameters that are at intersection of all three physical processes.

The particular structure of this distribution, with some clusters extending linearly away from the center and a set of outlying points, results in poor use of the plotting space, and high level of clustering near the center. For example, focussing on the blue cluster, we can see that it extends out along different directions, but it can be challenging to observe how the point move under the tour rotation, as overplotting becomes an issue when points move through the center. Here, the new display has a clear advantage, as illustrated with the selected views in Fig XXX.
