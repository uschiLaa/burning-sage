...
the curse of dimensionality in low dimensional projections of high dimensional data, and have shown its utility in applications in the natural sciences and machine learning. Our case studies highlight that the radial transformation can reduce the effects of data piling, and in combination with the grand tour our new sage display provides interpretable visualisation, while preserving global structure in high dimensional space. 

The sage display is easily incorporated into exploratory high dimensional data analysis
tasks: classification, clustering, and search. For classification tasks,
as shown in the sketches example, the sage display decreases the number of 
overlapping points between classes and provides better visual separation between
classes compared to the regular tour.  For clustering tasks, as shown in the 
single cell RNA-seq example, the sage display uncovered clusters that were 
originally obscured by data piling. Furthermore, unlike some
non-linear dimension reduction (NLDR) techniques like t-SNE, the sage display 
still gives the viewer an accurate assessment of the size of a cluster, 
and their relative orientation; we learned that yellow and blue clusters have
less separation in high dimensional space than the clustering algorithm suggests.
For search tasks, as shown in the pollen and pdfsense examples, the sage display
allows to find hidden signal that is concealed by the density of points around
the center of the projection. In the pdfsense example, by zooming in on the 
...
The applications also highlight that tuning is often required,
...
in the display, a well known problem for other NLDR methods as well.
...
<!--
Our parameter tuning example highlights this: the sage display is 
linear up to a  given effective dimension; the default sage display, preserves 
the correlation between the variables which is broken once points get moved
to the edge of the hypersphere as $\gamma$ and $R$ are changed.
-->
Given
...
Currently, the 
sage display is naive to information in high dimensional space that is not 
