...
```{r cdf, fig.cap="Comparing the relative volume of a $p$ dimensional hypersphere captured within a radius $r$, in the $p$ dimensional space (left) and in a 2D projection (right), for $p=3, 10, 100$. While most of the volume is squeezed towards larger values of $r$ as $p$ increases, the projected volume gets concentrated near the center instead.", fig.height=2.2, fig.width=6, out.width = "90%", fig.align = "center"}
...
## Parametrised rescaling {#sec:params}

Eq. \ref{eq:resc} is fixed for a given input dataset, by evaluating the number of dimensions $p$ and the maximum distance from the center $R$. However, we may wish to tune the rescaling. For example, the dimension of the input might not reflect the intrinsic dimensionality of the dataset. This could be the case when dimension reduction was used prior to visualisation, e.g. displaying only the first few principal components. In this case the effective dimensionality $p_{eff}$ is likely between the original dimensionality and the selected number of principal components, since some directions would be considered pure noise, while others might still carry relevant information.
