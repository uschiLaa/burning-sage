...
p1 <- ggplot() +
  ...
  geom_circle(aes(x0= 0, y0=0, r = xp), color="red", fill = "blue", alpha=0.5) +
  ...
  annotate("text", x = 0, y = 0.75, label=TeX("$V_{2D}(r; p, R)$")) + 
  annotate("text", x = 0, y = -xp * 0.75, label=TeX("$V(r; p, R)$")) + 
  annotate("text", x = 0.65, y = 0.5, label="V(R,p)") +
  theme_void() +
  coord_fixed()
