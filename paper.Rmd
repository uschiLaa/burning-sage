...
proj_points <- as_tibble(rbind(p3, p10, p100)) %>%
  mutate(p = c(rep(3, n), rep(10, n), rep(100, n)))


ggplot(proj_points, aes(x, y)) +
  #geom_point(alpha=0.02) + 
  #geom_density2d(bins = 7, contour_var = "ndensity") +
  geom_hex(bins = 12, aes(fill=log(..count..))) +
  #geom_hex(bins = 12) +
...
### a radial binning
n <- 5
r_breaks <- seq(0, 1, length.out = n+1)
proj_points_rbin <- proj_points %>%
  mutate(r = sqrt(x^2 + y^2)) %>%
  mutate(bin = cut(r, r_breaks)) %>%
  mutate(bin_up = as.numeric( sub("[^,]*,([^]]*)\\]", "\\1", bin))) %>%
  group_by(p) %>%
  count(bin_up) %>%
  arrange(-bin_up)

ggplot(proj_points_rbin) +
  geom_circle(aes(x0= 0, y0=0, r = bin_up, fill=log(n))) +
  scale_fill_viridis_c() +
  facet_wrap(~p, labeller = labeller(name = lbl)) +
  coord_fixed() +
  theme_bw()
...
XXX maybe a better picture is going from a 2D circle to a 1D projection?
...
ggplot() +
  ...
  geom_area(aes(x=x_outer1, y=yp_outer1), fill = "blue", alpha = 0.5) +
  geom_area(aes(x=x_outer1, y=ym_outer1), fill = "blue", alpha = 0.5) +
  geom_area(aes(x=x_outer2, y=yp_outer2), fill = "blue", alpha = 0.5) +
  geom_area(aes(x=x_outer2, y=ym_outer2), fill = "blue", alpha = 0.5) +
  geom_area(aes(x=x_outer1-1+xp, y=yp_outer1), fill = "blue", alpha = 0.5) +
  geom_area(aes(x=x_outer1-1+xp, y=ym_outer1), fill = "blue", alpha = 0.5) +
  geom_area(aes(x=x_outer2+1-xp, y=yp_outer2), fill = "blue", alpha = 0.5) +
  geom_area(aes(x=x_outer2+1-xp, y=ym_outer2), fill = "blue", alpha = 0.5) +
  theme_classic() +
  xlab("x") + ylab("y") +
  coord_fixed()
...
The aim of the algorithm is to redistribute the projected volume such that equal relative areas on the disk, as given by $v_{2} (r, p=2, R)= v_p(r, p=2, R) = (r/R)^2$, contain equal relative projected volume, given by $v_{2} (r, p, R)$. This is achieved through a transformation of the projected radius that can be defined for any $r\in[0,R]$, and is applied to the projected data points in the plane, $x = (x_1, x_2)$. We work with polar coordinates and represent the data points as $x = (r_x, \theta_x)$. The angular component $\theta_x$ is uniform for this distribution, by the rotation invariance of the sphere, and thus does not need to be transformed. The radial component $r_x$ is transformed in two steps.

The first transformation is to replace $r_x$ with $v_{2} (r_x, p, R)$. Since this is the radial CDF of the assumed underlying distribution, we expect that $v_{2} (r_x, p, R)$ is approximately uniformly distributed in radius. We then transform $v_{2} (r_x, p, R)$ using the inverse of $v_{2} (r_x, 2, R)$, to go from a uniform distribution in radius to a uniform distribution in area of the disk. This inverese is defined via
\begin{equation}
v_2(v_2^{inv}(r_x, 2, R), 2, R) = \left(\frac{v_2^{inv}(r_x, 2, R)}{R}\right)^2= r_x
\end{equation}
and thus
\begin{equation}
v_2^{inv}(r_x, 2, R) = R \sqrt{r_x}.
\end{equation}

The full radial transformation is therefore given by 
\begin{equation}
r'_x = v_2^{inv} (v_2(r_x, p, R), 2, R) =  R \sqrt{v_2(r_x, p, R)} = R \sqrt{1-\left(1-\left(\frac{r_x}{R}\right)^2\right)^{p/2}}.
\label{eq:resc}
\end{equation}

The relation between $r'_x$ and $r_x$ depends on the number of dimensions $p$, and is illustrated for selected values in Figure \ref{fig:radii}. We see that the transformation is approximately linear near the center. As $p$ increases it becomes non-linear faster, and e.g. for $p=10$ the points with radius $r_x>0.5$ will already be highly distorted and pushed out towards the last eighth in $r'_x$. Figure \ref{fig:circles} demonstrates this for different values of $p$ by showing equidistant circles for which the radius has been transformed according to Eq. \ref{eq:resc}.

```{r radii, fig.cap="Relation between $r_x$ and $r'_x$ for different values of $p$ and assuming $R=1$. The scaling is approximately linear near the center, but leads to distortion at large radii when $p$ is large.", fig.height=3, fig.width=6, out.width="60%", fig.align = "center"}
...
ggplot(data = data.frame(x = c(0,1)), mapping = aes(x = x)) +
  ...
  xlab(expression(r[x])) + ylab(expression(r[x]*minute)) + theme_bw()
...
We therefore allow trimming of the projected observations, using $R$ as a free parameter of the display function. When selecting a value $R$ that is smaller than the maximum distance from the center, we need to ensure that the projected radius of points is always smaller than $R$, by trimming $r_x$ as
\begin{equation}
r_x^{\mathrm{trim}} = \min(r_x, R)
\label{eq:cutR}
\end{equation}
...
We allow tuning $p_{\mathrm{eff}} = s p$ by selecting the scaling parameter $s$. By default, $s=1$ and $p_{\mathrm{eff}}=p$. When $s<1$ the rescaling will be softer, and $s>1$ results in more aggressive rescaling than suggested by $p$ alone. Note that when $p_{\mathrm{eff}} < 2$ we actually invert the behavior and shift the focus away from the center, in general this is not wanted.
...
4. To fit the final projection onto the plotting canvas ranging between $[-1,1]$, we rescale each mapped observation $y$ using the `half_range` parameter.
...
tourr::animate(
  ...
  display = display_sage(s, R, half_range)
  )
```

and introduces the scaling parameter `s` for computing $p_{\mathrm{eff}}$, and the overall scale `R` used for trimming. Both these parameters are described in Section \ref{sec:params} above. The ratio of `half_range` and `R` sets the scale for fitting the displayed data on the plotting canvas, by default `half_range` = `R` and we apply the scaling factor $0.9$ to contain the projected points within the display. When adjusting `R` the user should take care to adjust the `half_range` parameter accordingly.
