...
Something about usual analysis methods too, and how this helps to complement these.
- *Summary of data pre-processing*:
- *What is learned from the sage tour that is hidden in a regular tour*:

find one group that is clumped then focus on that for the sage tour

## Classifying hand-sketches

We can use the new display to look at different distribution of images from Google QuickDraw (XXX REFERENCE). These are $28\times28=784$ pixel grey scale data that are available publicly. In this example large dataset we use a small subset of three types of sketches (banana, cactus, crab) and observations (1000 sketches from each class) and see if we can separate the classes in the high-dimensional parameter space. 

We reduce the dimensionality from 784 variables to the five first PCs, which captures approximately 20 per cent of the variation of the data. Before applying the tour we rescale each component to have mean zero and unit variance. 

Figure xxx shows the grand tour on the PCs, where green points correspond to the banana class, orange points represent the cactcus class and purple points are the crab class. 
(frame 1 for grand tour, frame 36 for sage tour)

- both displays the orange points are more near the center, while in the grand tour display points from other classes overlap the orange points, while they are more seperated in the sage tour

- it is easier to see in the sage tour that the centers of each cluster are separated

- it is easier to see in the sage tour that the banana class has two sub groupings 

- in this particular case it helps with overplotting



## Comparing physics experiments: PDFSense

Data were obtained from CT14HERA2 parton distribution function fits as used in Laa et al., 2018. There are 28 directions in the parameter
...
(maximum likelihood estimate) fit of the function. Each observation has all predictions of the corresponding measurement from an experiment. Each point
is colored according to different underlying processes in the experiment.

- more detail by blowing out the center of the distribution



## Tuning the parameters: Pollen

The classical pollen data allows us to better demonstrate the use of the trimming and tuning parameters. The five dimensional data set was simulated by David Coleman of RCA Labs, for the Joint Statistics Meetings 1986 Data Expo [@pollen], and is an example of a hidden structure near the centre of a distribution. The data are standardised by centering and scaling such that the standard deviation of each variable is equal to one. The standard scatter display and the sage display with default settings are shown in Figure \ref{fig:pollen}, for a view found along a grand tour path. We see that neither display can reveal the structure. We next use either $\gamma$, $R$ or a combination of the two to zoom in further near the center, see Figure \ref{fig:pollen-tuned}. There is approximate equivalence between the results obtained using either tuning or trimming, and each of the views clearly reveals the word "EUREKA" hidden in the distribution. After scaling the maximum radius from the center (i.e. the true or default $R$ parameter) is $R=6.6$, and the settings used in Figure \ref{fig:pollen-tuned} are: $(R=6.6,\gamma=20)$ (left), $(R=1, \gamma=1)$ (middle), $(R=3, \gamma=5)$ (right).

\begin{figure}
\centering
\includegraphics[width=0.4\textwidth]{png/pollen_xy-88.png}
\includegraphics[width=0.4\textwidth]{png/pollen_sage-88.png}
\caption{Selected view of the pollen data in the standard xy-scatter display (left) and new sage display with default parameters (right). The structure is only visible near the center and cannot be seen in either display.}
\label{fig:pollen}
\end{figure}
...
\includegraphics[width=0.3\textwidth]{png/pollen_sage_gam5_R3-88.png}
\caption{Selected view of the pollen data in the new sage display setting $\gamma=20$ (left), $R=1$ (middle) or $(\gamma=5, R=3)$ (right). We can tune either $\gamma$, $R$ or a combination of the two to reveal the word "EUREKA" near the center of the distribution.}
\label{fig:pollen-tuned}
\end{figure}
